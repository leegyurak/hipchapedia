...
interface LyricsSearchFormProps {
  onSubmit: (data: LyricsSearchRequest) => void;
  isLoading?: boolean;
}

export default function LyricsSearchForm({ onSubmit, isLoading = false }: LyricsSearchFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-4xl mx-auto space-y-4 sm:space-y-6 px-3 sm:px-4">
      <div className="space-y-1.5 sm:space-y-2">
        <label htmlFor="artist" className="block text-sm sm:text-base font-semibold text-white">
          아티스트
        </label>
        <input
          id="artist"
